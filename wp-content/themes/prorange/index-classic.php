<?php
/**
 * The template for homepage posts with "Classic" style
 *
 * @package WordPress
 * @subpackage PRORANGE
 * @since PRORANGE 1.0
 */

prorange_storage_set('blog_archive', true);

// Load scripts for 'Masonry' layout
if (substr(prorange_get_theme_option('blog_style'), 0, 7) == 'masonry') {
	wp_enqueue_script( 'classie', prorange_get_file_url('js/theme.gallery/classie.min.js'), array(), null, true );
	wp_enqueue_script( 'imagesloaded', prorange_get_file_url('js/theme.gallery/imagesloaded.min.js'), array(), null, true );
	wp_enqueue_script( 'masonry', prorange_get_file_url('js/theme.gallery/masonry.min.js'), array(), null, true );
	wp_enqueue_script( 'prorange-gallery-script', prorange_get_file_url('js/theme.gallery/theme.gallery.js'), array(), null, true );
}

get_header(); 

if (have_posts()) {

	echo get_query_var('blog_archive_start');

	$prorange_classes = 'posts_container '
						. (substr(prorange_get_theme_option('blog_style'), 0, 7) == 'classic' ? 'columns_wrap' : 'masonry_wrap');
	$prorange_stickies = is_home() ? get_option( 'sticky_posts' ) : false;
	$prorange_sticky_out = prorange_get_theme_option('sticky_style')=='columns' 
							&& is_array($prorange_stickies) && count($prorange_stickies) > 0 && get_query_var( 'paged' ) < 1;
	if ($prorange_sticky_out) {
		?><div class="sticky_wrap columns_wrap"><?php	
	}
	if (!$prorange_sticky_out) {
		if (prorange_get_theme_option('first_post_large') && !is_paged() && !in_array(prorange_get_theme_option('body_style'), array('fullwide', 'fullscreen'))) {
			the_post();
			get_template_part( 'content', 'excerpt' );
		}
		
		?><div class="<?php echo esc_attr($prorange_classes); ?>"><?php
	}
	while ( have_posts() ) { the_post(); 
		if ($prorange_sticky_out && !is_sticky()) {
			$prorange_sticky_out = false;
			?></div><div class="<?php echo esc_attr($prorange_classes); ?>"><?php
		}
		get_template_part( 'content', $prorange_sticky_out && is_sticky() ? 'sticky' : 'classic' );
	}
	
	?></div><?php

	prorange_show_pagination();

	echo get_query_var('blog_archive_end');

} else {

	if ( is_search() )
		get_template_part( 'content', 'none-search' );
	else
		get_template_part( 'content', 'none-archive' );

}

get_footer();
?>