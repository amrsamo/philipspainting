...
foreach ($args['values'] as $v) {
    ...
    $im = '';
    if (!empty($v['image'])) {
        $v['image'] = trx_addons_get_attachment_url($v['image'], trx_addons_get_thumb_size('medium'));
        $attr = trx_addons_getimagesize($v['image']);
        $im = $im . '<img src="' . esc_url($v['image']) .'" alt=""' . (!empty($attr[3]) ? ' ' . trim($attr[3]) : '') . '>';
    }
	$data .= ($args['columns'] > 0 ? '<div class="sc_skills_column '.esc_attr(trx_addons_get_column_class(1, $args['columns'])).'">' : '')
			. '<div class="sc_skills_item_wrap">'
                . (!empty($im) ? '<div class="counter-image">'.$im . '</div><div class="counter-body">' : '')
