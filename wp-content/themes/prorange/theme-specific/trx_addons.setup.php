...
------------------------------------------------------------------------------- */
if (!function_exists('prorange_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'prorange_trx_addons_theme_specific_components');
	function prorange_trx_addons_theme_specific_components($enable=false) {
		return false;
	}
}

if (!function_exists('prorange_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'prorange_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'prorange_trx_addons_theme_specific_setup1', 8 );
	function prorange_trx_addons_theme_specific_setup1() {
		if (prorange_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',				'prorange_trx_addons_cv_enable');
			add_filter( 'trx_addons_cpt_list',				'prorange_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',				'prorange_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',			'prorange_trx_addons_widgets_list');
...
// CV
if ( !function_exists( 'prorange_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'prorange_trx_addons_cv_enable');
	function prorange_trx_addons_cv_enable($enable=false) {
...
// CPT
if ( !function_exists( 'prorange_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'prorange_trx_addons_cpt_list');
	function prorange_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'prorange_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'prorange_trx_addons_sc_list');
	function prorange_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'prorange_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'prorange_trx_addons_widgets_list');
	function prorange_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'prorange_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'prorange_trx_addons_menu_cache');
	function prorange_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('prorange_trx_addons_localize_script')) {
	...
	function prorange_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = prorange_get_scheme_color('alter_link');
		return $arr;
	}
}

// Setup internal plugin's parameters
if (!function_exists('prorange_trx_addons_options')) {
	add_action( 'trx_addons_filter_after_load_options', 'prorange_trx_addons_options');
	function prorange_trx_addons_options($options) {
		// Type of socials:
...
if ( !function_exists( 'prorange_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'prorange_trx_addons_sc_type', 10, 2);
	function prorange_trx_addons_sc_type($list, $sc) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
        if (in_array($sc, array('trx_sc_button'))) {
            $list[ esc_html__('Inverse', 'prorange') ] = 'inverse';
        }
		return $list;
	}
}
...
if ( !function_exists( 'prorange_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'prorange_trx_addons_sc_atts', 10, 2);
	function prorange_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'prorange_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'prorange_trx_addons_sc_map', 10, 2);
	function prorange_trx_addons_sc_map($params, $sc) {
        ...
        if (in_array($sc, array('trx_sc_skills'))) {
            $params['params'][8]['params'][] = array(
                "param_name" => "image",
                "heading" => esc_html__("Image", 'prorange'),
                "description" => wp_kses_data( __("Select or upload image or specify URL from other site (only for counter)", 'prorange') ),
                "type" => "attach_image"
            );
        }
...
if ( !function_exists( 'prorange_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'prorange_trx_addons_sc_output', 10, 4);
	function prorange_trx_addons_sc_output($output, $sc, $atts, $content) {
...
if ( !function_exists( 'prorange_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'prorange_trx_addons_sc_item_title_tag');
	function prorange_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'prorange_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'prorange_trx_addons_sc_item_button_args');
	function prorange_trx_addons_sc_item_button_args($args, $sc='') {
...
if ( !function_exists( 'prorange_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'prorange_trx_addons_slider_title', 10, 2 );
	function prorange_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'prorange_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'prorange_trx_addons_sc_googlemap_styles');
	function prorange_trx_addons_sc_googlemap_styles($list) {
		$list[esc_html__('Dark', 'prorange')] = 'dark';
        $list[esc_html__('Grey', 'prorange')] = 'grey';
		return $list;
	}
}
...
if ( !function_exists( 'prorange_trx_addons_editor_init' ) ) {
	if (is_admin()) add_filter( 'tiny_mce_before_init', 'prorange_trx_addons_editor_init', 11);
	function prorange_trx_addons_editor_init($opt) {
		if (prorange_exists_trx_addons()) {
			...
			if (false && !empty($opt['style_formats'])) {
				...
				if (is_array($style_formats) && count($style_formats)>0 ) {
					foreach ($style_formats as $k=>$v) {
						if ( $v['title'] == esc_html__('List styles', 'prorange') ) {
							$style_formats[$k]['items'][] = array(
										'title' => esc_html__('Arrow', 'prorange'),
										'selector' => 'ul',
...
if ( !function_exists( 'prorange_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'prorange_filter_allow_override_header_image', 'prorange_trx_addons_allow_override_header_image' );
	function prorange_trx_addons_allow_override_header_image($allow) {
		return prorange_is_team_page() || prorange_is_portfolio_page() ? false : $allow;
	}
}

// Hide sidebar on the team and portfolio pages
if ( !function_exists( 'prorange_trx_addons_sidebar_present' ) ) {
	add_filter( 'prorange_filter_sidebar_present', 'prorange_trx_addons_sidebar_present' );
	function prorange_trx_addons_sidebar_present($present) {
		return !is_single() && (prorange_is_team_page() || prorange_is_portfolio_page()) ? false : $present;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'prorange_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'prorange_trx_addons_thumb_size', 10, 2);
	function prorange_trx_addons_thumb_size($thumb_size='', $type='') {
...
if (!function_exists('prorange_trx_addons_meta_box_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'prorange_trx_addons_meta_box_fields', 10, 2);
	function prorange_trx_addons_meta_box_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'prorange'),
				"desc" => wp_kses_data( __("Team member's email", 'prorange') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
