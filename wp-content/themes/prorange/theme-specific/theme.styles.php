...
if (!function_exists('prorange_customizer_get_css')) {

	function prorange_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = prorange_customizer_add_theme_fonts($fonts);
...
CSS;
			$rez = apply_filters('prorange_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];

			
			// Border radius
			//--------------------------------------
			$rad = prorange_get_border_radius();
			$rad50 = ' '.$rad != ' 0' ? '50%' : 0;
			$css['fonts'] .= <<<CSS
...
CSS;
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(prorange_get_list_schemes()) : array($only_scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = prorange_get_scheme_colors($scheme);
...
table > tbody > tr:nth-child(3n) > td {
...
/* Text fields */
::-webkit-input-placeholder { color: {$colors['text_dark_078']}; }
::-moz-placeholder          { color: {$colors['text_dark_078']}; }
:-ms-input-placeholder      { color: {$colors['text_dark_078']}; }
...
.sticky.post_item .post_meta_item,
.sticky.post_item .post_meta_item:before,
.sticky.post_item .post_meta_item a {
    color: {$colors['inverse_link']};
}
.sticky.post_item a.post_meta_item:hover,
.sticky.post_item .post_meta_item a:hover:before,
.sticky.post_item .post_meta_item a:hover {
    color: {$colors['inverse_link_06']};
}
...
.prorange_tabs .prorange_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
.prorange_tabs .prorange_tabs_titles li.ui-state-active a {
...
.single-meta .post_meta_item.post_categories a {
    color: {$colors['extra_light']};
}
.single-meta .post_meta_item.post_categories a:hover {
    color: {$colors['text_hover']};
}
/* Single post navi */
...
.sc_item_title.sc_align_left:after {
    background-color: {$colors['text_hover']};
}
/* Comments */
.comments_list_wrap,
...
.post_meta .single-meta .post_meta_item a:hover {
    background-color: {$colors['text_link']};
    color: {$colors['inverse_link']};
}
...
.vc_row-has-fill input[type="text"],
.vc_row-has-fill input[type="email"] {
    background-color: {$colors['bg_color']};
}
.post-bottom {
    border-color: {$colors['text_hover']};
...
.sidebar .widget.widget_search .widget_title:before,
...
.top_panel.top_panel_default .scheme_self.top_panel_title {
...
.scheme_dark .widget li.recentcomments:before, .scheme_dark .widget li.recentcomments a,
.scheme_dark .widget li a {
    color: {$colors['inverse_link_06']};
}
.scheme_dark .widget li.recentcomments + li.recentcomments, .scheme_dark .widget .post_item + .post_item {
    border-color: {$colors['inverse_link_06']};
}
...
CSS;
					...
					$rez = apply_filters('prorange_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'prorange_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>